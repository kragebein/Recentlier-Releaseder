...
def collect():
    ...
    if dump and found_item is False:
        ...
        collector.tracklist.update({'tracks': dump['tracks'], 'albums': dump['albums']}) # Include old dump
        for i in albums:
        ...
        collector.tracklist.update({'username': collector.username, 'tracks': track_data, 'albums': albums})
        collector.updateplaylist()
        writedump()
        return True
    else:
